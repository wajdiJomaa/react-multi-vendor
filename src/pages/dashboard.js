...
const Dashboard = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await axios.get(
        "http://127.0.0.1:8000/api/get_vendor_products/1"
      );
  ...
  const fetchReviews = async () => {
    try {
      ...
      const nonEmptyReviews = response.data.filter(
        (review) => review.comment !== ""
      );
      const totalReviews = nonEmptyReviews.length;
      const positiveReviews = nonEmptyReviews.filter(
        (review) => review.rating >= 4
      ).length;
      const negativeReviews = nonEmptyReviews.filter(
        (review) => review.rating < 4
      ).length;
      setReviewsChartData([
  ...
  const handleDeleteProduct = async (productId) => {
    try {
      await axios.delete(
        `http://127.0.0.1:8000/api/delete_product/${productId}`
      );
  ...
  const columns = [
    ...
    { title: "Description", dataIndex: "description", key: "description" },
    {
      ...
      key: "image",
      render: (image) => (
        <img src={image} alt="Product" className="product-image1" />
      ),
    },
    {
      title: "Actions",
  ...
  const renderCircularDiagram = () => {
    if (reviewsChartData) {
      ...
      const totalReviews = nonEmptyReviews.length;
      const goodReviews = nonEmptyReviews.filter(
